...
```{r}
outdir <- "vtamR_demo_out/Tutorial_mfzr_plate1"
...
asv_list <- system.file("extdata/demo/updated_ASV_list_mfzr.csv", package = "vtamR") # all ASV of MFZR
```
...
Since the NCBI reference sequences were correctly chosen, this indicates that the ZFZR primer pair did not amplify *Phoxinus phoxinus*.
...
head(suggest_sample_cutoff_df, n=12)
```
...
Therefore, the true number of false negatives is 2, and the sensitivity is:
