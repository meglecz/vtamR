---
title: "How to make a mock_composition file - v0.3.0"
output: 
  rmarkdown::html_vignette:
    toc: true
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{How to make a mock_composition file - v0.3.0}
...
This function provides a convenient and often faster way to build the `mock_composition` file.
...
* **Taxonomy file**
  ...
  (see [TaxAssign reference data base](installation.html#TaxAssign_reference_data_base)).
...
2. **Assigns taxonomy** to all ASVs detected in the mock samples using this custom database.
   Since the database is small, the assignment is fast. However, if some 
   reference sequences are mislabeled in the source database, the corresponding 
   ASVs may remain unassigned.
...
taxonomy <- "COInr_for_vtam_2025_05_23_dbV5/COInr_for_vtam_taxonomy.tsv" # downloaded from OSF [TaxAssign reference data base](installation.html#TaxAssign_reference_data_base)
...
**Note**: Do not forget to specify `blast_path` is BLAST is not in your PATH (see [Installation](installation.html))
...
TaxAssign will assign all asv in the input csv file or data frame (`read_count_file`).
...
```{r}
asv_tax <- TaxAssign(asv=read_count_mock, 
                     taxonomy=taxonomy, 
                     blast_db=blast_db
                     )
```
