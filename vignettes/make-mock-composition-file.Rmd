...
### Set parametres and access de themo files

The demo files bellow are included in the vtamR package, hence the use of 
`system.file()`. 
When using your own data just enter your file names.

TaxAssign accepts both a csv file or a data frame with `asv_id` and `asv` columns.
Here, we use a demo csv file, but you will probably use the filtered 
`read_count_df` data frame. See [Tutorial](tutorial-vtamr-pipeline.html)
